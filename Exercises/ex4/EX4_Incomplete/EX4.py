...
from python_rf_course_utils.qt.pyqt2python import h_gui

# EX4: import the multitone module from the python_rf_course_utils.arb directory (slide 2-7)

from python_rf_course_utils.arb.multitone import multitone
...
class LabDemoMxgControl(QMainWindow):
    def __init__(self):
        ...
        self.h_gui = dict(
            ...
            Ntones              = h_gui(self.dial              , self.cb_multitone_update  ),
            Multitone           = h_gui(self.pushButton_4       , self.cb_multitone_on_off ))
    ...
    def cb_connect(self):
        if self.sender().isChecked(): # self.h_gui['Connect'].obj.isChecked():
            ...
            try:
                ...
                # Read the signal generator status and update the GUI (RF On/Off, Modulation On/Off,Pout and Fc - 208)
                ...
                idn = self.sig_gen.query('*IDN?')
                company_name, model_number, serial_number, firmware_revision = idn.strip().split(',')
